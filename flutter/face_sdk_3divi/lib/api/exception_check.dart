...
Pointer<Pointer<Void>> _getException() {
  ...
  if (exception.address == nullptr.address) throw Exception("_getException: Failed to allocate a 1 byte");
...
void checkException(Pointer<Pointer<Void>> exception, DynamicLibrary dll_handle) {
  if (exception.value.address != nullptr.address) {
    ...
    final check_exception_what = dll_handle.lookupFunction<_exceptionWhat, _exceptionWhat>(_c_namespace + 'apiException_what');
