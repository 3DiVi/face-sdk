...
def help_message():
    message = f"\n This program is an example of the Mask Estimator Processing block integration \n Usage: " \
...
def mask_estimator(input_image, sdk_path, use_cuda):
    ...
    mask_config = {  # mask block configuration parameters
        "unit_type": "MASK_ESTIMATOR",  # required parameter
        "model_path": sdk_path + "share/faceattributes/mask.enc",  # required
        "use_cuda": use_cuda,  # optional
        "ONNXRuntime": {
            "library_path": sdk_onnx_path  # optional
        }
    }
    mask_block = service.create_processing_block(mask_config)  # create mask estimation processing block
    ...
    picture = capturer_detection(service, img, mask_block)
    # or
    # picture = detector_detection(service, img, mask_block, sdk_path, sdk_onnx_path)
...
if __name__ == "__main__":
    ...
    mask_estimator(args.input_image, args.sdk_path, args.use_cuda)
