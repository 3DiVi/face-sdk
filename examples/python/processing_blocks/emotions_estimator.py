...
def help_message():
    message = f"\n This program is an example of the Emotion Estimator Processing block integration \n Usage: " \
...
def emotions_estimator(input_image, sdk_path, use_cuda):
    ...
    emotions_config = {  # emotions block configuration parameters
        "unit_type": "EMOTION_ESTIMATOR",  # required parameter
        "model_path": sdk_path + "/share/faceanalysis/emotion.enc",  # required
        "use_cuda": use_cuda,  # optional
        "ONNXRuntime": {
            "library_path": sdk_onnx_path  # optional
        }
    }
    emotions_block = service.create_processing_block(emotions_config)  # create emotions estimation processing block
    ...
    picture = capturer_detection(service, img, emotions_block)
    # or
    # picture = detector_detection(service, img, emotions_block, sdk_path, sdk_onnx_path)
...
if __name__ == "__main__":
    ...
    emotions_estimator(args.input_image, args.sdk_path, args.use_cuda)
