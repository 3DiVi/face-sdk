...
def help_message():
    message = f"\n This program is an example of the Pose Processing blocks integration \n Usage: " \
...
def parse_args():  # launch parameters
    parser = argparse.ArgumentParser(description="Pose Estimator Processing Block Example")
    ...
    parser.add_argument("--modification", type=str, default="heavy")
...
def draw_keypoints(obj: Context, image: np.ndarray):
    ...
    for bone in bone_map:  # get bones coordinates from the ioData container
        point_x1 = int(keypoints[bone[0]]["proj"][0].get_double() * width)
        point_y1 = int(keypoints[bone[0]]["proj"][1].get_double() * height)

        point_x2 = int(keypoints[bone[1]]["proj"][0].get_double() * width)
        point_y2 = int(keypoints[bone[1]]["proj"][1].get_double() * height)

        cv2.line(image, (point_x1, point_y1), (point_x2, point_y2), (255, 255, 0), 1,
...
def create_pose_estimator(modification: str, use_cuda: bool) -> ProcessingBlock:
    return service.create_processing_block(service.create_context({
...
if __name__ == "__main__":
    ...
    pipeline: Dict[str, ProcessingBlock] = {
        ...
        "pose_estimator": create_pose_estimator(args.modification, args.use_cuda)
    }
